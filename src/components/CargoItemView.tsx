...
export const CargoItemView: React.FC<Props> = ({ item }) => {
  ...
  if (item.center) {
    ...
    posY = Number(item.position.z) + visualYOffset;
    posZ = Number(item.position.y) + visualZOffset;
  }

  const position: [number, number, number] = [posX, posY, posZ];
  const rotation: [number, number, number] = [Number(item.rotation?.x || 0), Number(item.rotation?.y || 0), Number(item.rotation?.z || 0)];
  const radius = Number(item.dimensions?.width || 0) / 2;
  const cylHeight = Number(item.dimensions?.height || 0);
  const boxArgs: [number, number, number] = [Number(item.dimensions?.width || 0), Number(item.dimensions?.height || 0), Number(item.dimensions?.length || 0)];

  return (
    <group>
      <mesh ref={meshRef} position={position} rotation={rotation} onPointerOver={(e) => { e.stopPropagation(); setHover(true); }} onPointerOut={() => setHover(false)}>
        {item.type === 'cylinder' ? (
          <>
            <cylinderGeometry args={[radius, radius, cylHeight, 32]} />
            <meshStandardMaterial color={hovered ? '#ffffff' : color} roughness={0.5} metalness={0.1} opacity={Number(item.opacity ?? 1)} transparent={Number(item.opacity ?? 1) < 1} />
            <lineSegments>
      ...
      {hovered && (
        <Html position={[0, (item.type === 'cylinder' ? cylHeight/2 : Number(item.dimensions?.height)/2) + 10, 0]}>
          <div className="bg-black/80 text-white text-xs p-1 rounded whitespace-nowrap pointer-events-none z-50">
            {item.name} <br/> {Math.round(radius*2)}x{Math.round(cylHeight)}
          </div>
        </Html>
      )}
    </group>
